...
const AddTransactionDialog = ({ open, onClose, type = 'sale', sale = null }) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      // If editing a sale, pre-select the customer/supplier
      if (sale) {
        setSelectedParty(sale.customer); // Assumes sale.customer is the full customer object
        setSearchInputValue(sale.customer?.customerName || '');
        setInvoiceDialogOpen(true); // Open CreateSaleDialog immediately for editing
      }
    } else {
    ...
    <>
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth fullScreen={isMobile}>
        <DialogTitle>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Typography variant="h6">
              {sale ? `Edit ${type === 'sale' ? 'Sale' : 'Purchase'}` : `Add ${type === 'sale' ? 'Sale' : 'Purchase'}`}
            </Typography>
            <IconButton
              aria-label="close"
              onClick={onClose}
        ...
        <DialogContent>
          ...
          {!sale && ( // Hide Autocomplete when editing
            <Box sx={{ minWidth: 300 }}>
              <Autocomplete
                options={accounts}
          ...
          <PartyPreview party={selectedParty || (sale ? sale.customer : null)} type={type} />
        </DialogContent>
      </Dialog>
      ...
      {selectedParty || sale ? (
        type === 'sale' ? (
          <CreateSaleDialog
            ...
            customer={selectedParty || (sale ? sale.customer : null)}
            sale={sale} // Pass the sale for editing
          />
        ) : (
          <CreatePurchaseDialog
            open={invoiceDialogOpen}
            onClose={handleInvoiceClose}
            supplier={selectedParty || (sale ? sale.supplier : null)}
            sale={sale} // Assuming CreatePurchaseDialog can handle updates similarly
          />
        )
      ) : null}
